...
public interface DeviceAttrInfoMapper extends BaseMapper<DeviceAttrInfo> {
    ...
    @Select({
            ...
            "COALESCE(MAX(totalFlightSorties) - MIN(CASE WHEN totalFlightSorties IS NOT NULL AND totalFlightSorties &lt;&gt; 0 THEN totalFlightSorties END), 0) AS flightSortiesInRange, ",
            "COALESCE(MAX(totalFlightTime) - MIN(CASE WHEN totalFlightTime IS NOT NULL AND totalFlightTime &lt;&gt; 0 THEN totalFlightTime END), 0) AS flightTimeSecondsInRange, ",
            "COALESCE(MAX(totalFlightDistance) - MIN(CASE WHEN totalFlightDistance IS NOT NULL AND totalFlightDistance &lt;&gt; 0 THEN totalFlightDistance END), 0) AS flightDistanceMetersInRange ",
            "FROM (",
            "SELECT ",
